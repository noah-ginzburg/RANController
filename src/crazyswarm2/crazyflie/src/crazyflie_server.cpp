...
// Helper class to convert crazyflie_cpp logging messages to ROS logging
// messages
...
private:
  ...
  struct logStatus
  {
    ...
    // battery related
    // Note that using BQ-deck/Bolt one can actually have two batteries at the
    // same time. vbat refers to the battery directly connected to the CF board
    // and might not reflect the "external" battery on BQ/Bolt builds
...
public:
  CrazyflieROS(const std::string &link_uri, const std::string &cf_type,
      ...
      : logger_(node->get_logger()), cf_logger_(logger_, "[" + name + "]"),
        ...
        last_on_latency_(std::chrono::steady_clock::now()), cfbc_(cfbc)
  {
    ...
    service_emergency_ = node->create_service<Empty>(
        name + "/emergency", std::bind(&CrazyflieROS::emergency, this, _1, _2),
        get_service_qos(), callback_group_cf_srv);
    service_start_trajectory_ = node->create_service<StartTrajectory>(
        name + "/start_trajectory",
        std::bind(&CrazyflieROS::start_trajectory, this, _1, _2),
        get_service_qos(), callback_group_cf_srv);
    service_takeoff_ = node->create_service<Takeoff>(
        name + "/takeoff", std::bind(&CrazyflieROS::takeoff, this, _1, _2),
        get_service_qos(), callback_group_cf_srv);
    service_land_ = node->create_service<Land>(
        name + "/land", std::bind(&CrazyflieROS::land, this, _1, _2),
        get_service_qos(), callback_group_cf_srv);
    service_go_to_ = node->create_service<GoTo>(
        name + "/go_to", std::bind(&CrazyflieROS::go_to, this, _1, _2),
        get_service_qos(), callback_group_cf_srv);
    service_upload_trajectory_ = node->create_service<UploadTrajectory>(
        name + "/upload_trajectory",
        std::bind(&CrazyflieROS::upload_trajectory, this, _1, _2),
        get_service_qos(), callback_group_cf_srv);
    service_notify_setpoints_stop_ = node->create_service<NotifySetpointsStop>(
        name + "/notify_setpoints_stop",
        std::bind(&CrazyflieROS::notify_setpoints_stop, this, _1, _2),
        get_service_qos(), callback_group_cf_srv);
    service_arm_ = node->create_service<Arm>(
        name + "/arm", std::bind(&CrazyflieROS::arm, this, _1, _2),
        get_service_qos(), callback_group_cf_srv);

    // Topics

    subscription_cmd_vel_legacy_ =
        node->create_subscription<geometry_msgs::msg::Twist>(
            name + "/cmd_vel_legacy", rclcpp::SystemDefaultsQoS(),
            std::bind(&CrazyflieROS::cmd_vel_legacy_changed, this, _1),
            sub_opt_cf_cmd);
    subscription_cmd_full_state_ =
        node->create_subscription<crazyflie_interfaces::msg::FullState>(
            name + "/cmd_full_state", rclcpp::SystemDefaultsQoS(),
            std::bind(&CrazyflieROS::cmd_full_state_changed, this, _1),
            sub_opt_cf_cmd);
    subscription_cmd_position_ =
        node->create_subscription<crazyflie_interfaces::msg::Position>(
            name + "/cmd_position", rclcpp::SystemDefaultsQoS(),
            std::bind(&CrazyflieROS::cmd_position_changed, this, _1),
            sub_opt_cf_cmd);
    subscription_cmd_velocity_world_ =
        node->create_subscription<crazyflie_interfaces::msg::VelocityWorld>(
            name + "/cmd_velocity_world", rclcpp::SystemDefaultsQoS(),
            std::bind(&CrazyflieROS::cmd_velocity_world_changed, this, _1),
            sub_opt_cf_cmd);
    subscription_cmd_hover_ =
        node->create_subscription<crazyflie_interfaces::msg::Hover>(
            name + "/cmd_hover", rclcpp::SystemDefaultsQoS(),
            std::bind(&CrazyflieROS::cmd_hover_changed, this, _1),
            sub_opt_cf_cmd);

    publisher_robot_description_ =
        node->create_publisher<std_msgs::msg::String>(
            name + "/robot_description", rclcpp::QoS(1).transient_local());
    {
      auto msg = std::make_unique<std_msgs::msg::String>();
      auto robot_desc = node->get_parameter("robot_description")
                            .get_parameter_value()
                            .get<std::string>();
    ...
    spin_timer_ = node->create_wall_timer(
        std::chrono::milliseconds(1), std::bind(&CrazyflieROS::spin_once, this),
        callback_group_cf_srv);

    // link statistics
    warning_freq_ = node->get_parameter("warnings.frequency")
                        .get_parameter_value()
                        .get<float>();
    max_latency_ =
        node->get_parameter("warnings.communication.max_unicast_latency")
            .get_parameter_value()
            .get<float>();
    min_ack_rate_ =
        node->get_parameter("warnings.communication.min_unicast_ack_rate")
            .get_parameter_value()
            .get<float>();
    min_unicast_receive_rate_ =
        node->get_parameter("warnings.communication.min_unicast_receive_rate")
            .get_parameter_value()
            .get<float>();
    min_broadcast_receive_rate_ =
        node->get_parameter("warnings.communication.min_broadcast_receive_rate")
            .get_parameter_value()
            .get<float>();
    publish_stats_ = node->get_parameter("warnings.communication.publish_stats")
                         .get_parameter_value()
                         .get<bool>();
    if (publish_stats_)
    {
      publisher_connection_stats_ = node->create_publisher<
          crazyflie_interfaces::msg::ConnectionStatisticsArray>(
          name + "/connection_statistics", 10);
    }

    if (warning_freq_ >= 0.0)
    {
      cf_.setLatencyCallback(
          std::bind(&CrazyflieROS::on_latency, this, std::placeholders::_1));
      link_statistics_timer_ = node->create_wall_timer(
          std::chrono::milliseconds((int)(1000.0 / warning_freq_)),
          std::bind(&CrazyflieROS::on_link_statistics_timer, this),
          callback_group_cf_srv);
    }
    ...
    auto update_map = [&parameter_overrides](
                          std::map<std::string, rclcpp::ParameterValue> &map,
                          const std::string &pattern)
    {
      for (const auto &i : parameter_overrides)
      {
    ...
    auto update_value = [&parameter_overrides](rclcpp::ParameterValue &value,
                                               const std::string &pattern)
    {
      for (const auto &i : parameter_overrides)
      {
    ...
    if (enable_parameters)
    {
      bool query_all_values_on_connect =
          node->get_parameter("firmware_params.query_all_values_on_connect")
              .get_parameter_value()
              .get<bool>();
      ...
      for (auto iter = cf_.paramsBegin(); iter != cf_.paramsEnd(); ++iter)
      {
        ...
        std::string paramName =
            name + ".params." + entry.group + "." + entry.name;
        switch (entry.type)
        {
        ...
        case Crazyflie::ParamTypeUint16:
          if (query_all_values_on_connect)
          {
            node->declare_parameter(paramName,
                                    cf_.getParam<uint16_t>(entry.id));
          }
          else
          {
            node->declare_parameter(paramName, rclcpp::PARAMETER_INTEGER);
        ...
        case Crazyflie::ParamTypeUint32:
          if (query_all_values_on_connect)
          {
            node->declare_parameter<int64_t>(paramName,
                                             cf_.getParam<uint32_t>(entry.id));
          }
          else
          {
            node->declare_parameter(paramName, rclcpp::PARAMETER_INTEGER);
        ...
        case Crazyflie::ParamTypeFloat:
          ...
          break;
        default:
          RCLCPP_WARN(logger_, "[%s] Unknown param type for %s/%s",
                      name_.c_str(), entry.group.c_str(), entry.name.c_str());
          break;
        }
        // If there is no such parameter in all, add it
        std::string allParamName =
            "all.params." + entry.group + "." + entry.name;
      ...
      RCLCPP_INFO(logger_, "[%s] reqParamTOC: %f s (%d params)", name_.c_str(),
                  elapsedSeconds1.count(), numParams);
      ...
      for (const auto &i : set_param_map)
      {
        std::string paramName =
            name + ".params." +
            std::regex_replace(i.first, std::regex("\\."), ".");
    ...
    // Reference Frame
    {
      ...
      // in the following order 1.) check all/reference_frame 2.) check
      ...
      update_value(reference_frame_value,
                   "robot_types." + cf_type + ".reference_frame");
      // check robots/<robot_name>/reference_frame
      update_value(reference_frame_value,
                   "robots." + name_ + ".reference_frame");

      // extract reference frame
      reference_frame_ = reference_frame_value.get<std::string>();

      RCLCPP_INFO(logger_, "[%s] ref frame: %s", name_.c_str(),
    ...
    // Logging
    {
      ...
      // robots/<robot_name>/firmware_logging where the higher order is used if
      // defined on multiple levels.
      update_map(log_config_map, "all.firmware_logging");
      // check robot_types/<type_name>/firmware_logging
      update_map(log_config_map,
                 "robot_types." + cf_type + ".firmware_logging");
      ...
      if (logging_enabled)
      {
        ...
        for (const auto &i : log_config_map)
        {
          ...
          {
            int freq =
                log_config_map["default_topics.pose.frequency"].get<int>();
            RCLCPP_INFO(logger_, "[%s] Logging to /pose at %d Hz",
                        name_.c_str(), freq);
            ...
            std::function<void(uint32_t, const logPose *)> cb =
                std::bind(&CrazyflieROS::on_logging_pose, this,
                          std::placeholders::_1, std::placeholders::_2);

            log_block_pose_.reset(
                new LogBlock<logPose>(&cf_,
                                      {{"stateEstimate", "x"},
                                       {"stateEstimate", "y"},
                                       {"stateEstimate", "z"},
                                       {"stateEstimateZ", "quat"}},
                                      cb));
            log_block_pose_->start(uint8_t(
                100.0f / (float)freq)); // this is in tens of milliseconds
          }
          else if (i.first.find("default_topics.scan") == 0)
          {
            int freq =
                log_config_map["default_topics.scan.frequency"].get<int>();
            RCLCPP_INFO(logger_, "[%s] Logging to /scan at %d Hz",
                        name_.c_str(), freq);
            ...
            std::function<void(uint32_t, const logScan *)> cb =
                std::bind(&CrazyflieROS::on_logging_scan, this,
                          std::placeholders::_1, std::placeholders::_2);
            ...
            log_block_scan_.reset(new LogBlock<logScan>(&cf_,
                                                        {{"range", "front"},
                                                         ...
                                                         {"range", "right"}},
                                                        cb));
            log_block_scan_->start(uint8_t(
                100.0f / (float)freq)); // this is in tens of milliseconds
          }
          else if (i.first.find("default_topics.odom") == 0)
          {
            int freq =
                log_config_map["default_topics.odom.frequency"].get<int>();
            RCLCPP_INFO(logger_, "[%s] Logging to /odom at %d Hz",
                        name_.c_str(), freq);

            publisher_odom_ = node->create_publisher<nav_msgs::msg::Odometry>(
                name + "/odom", 10);

            std::function<void(uint32_t, const logOdom *)> cb =
                std::bind(&CrazyflieROS::on_logging_odom, this,
                          std::placeholders::_1, std::placeholders::_2);

            log_block_odom_.reset(
                new LogBlock<logOdom>(&cf_,
                                      {
                                          {"stateEstimateZ", "x"},
                                          {"stateEstimateZ", "y"},
                                          {"stateEstimateZ", "z"},
                                      ...
                                      cb));
            log_block_odom_->start(uint8_t(
                100.0f / (float)freq)); // this is in tens of milliseconds
          }
          else if (i.first.find("default_topics.status") == 0)
          {
            int freq =
                log_config_map["default_topics.status.frequency"].get<int>();
            RCLCPP_INFO(logger_, "[%s] Logging to /status at %d Hz",
                        name_.c_str(), freq);

            publisher_status_ =
                node->create_publisher<crazyflie_interfaces::msg::Status>(
                    name + "/status", 10);

            std::function<void(uint32_t, const logStatus *)> cb =
                std::bind(&CrazyflieROS::on_logging_status, this,
                          std::placeholders::_1, std::placeholders::_2);
            ...
            for (auto iter = cf_.logVariablesBegin();
                 iter != cf_.logVariablesEnd(); ++iter)
            {
              auto entry = *iter;
            ...
            log_block_status_->start(uint8_t(
                100.0f / (float)freq)); // this is in tens of milliseconds
          }
                   ...
                   i.first.rfind(".vars") != std::string::npos)
          {
            std::string topic_name =
                i.first.substr(14, i.first.size() - 14 - 5);

            int freq =
                log_config_map["custom_topics." + topic_name + ".frequency"]
                    .get<int>();
            auto vars = log_config_map["custom_topics." + topic_name + ".vars"]
                            .get<std::vector<std::string>>();

            RCLCPP_INFO(logger_, "[%s] Logging to %s at %d Hz", name_.c_str(),
                        topic_name.c_str(), freq);
            ...
            std::function<void(uint32_t, const std::vector<float> *,
                               void *userData)>
                cb = std::bind(&CrazyflieROS::on_logging_custom, this,
                               std::placeholders::_1, std::placeholders::_2,
                               std::placeholders::_3);

            log_blocks_generic_.emplace_back(new LogBlockGeneric(
                &cf_, vars, (void *)&publishers_generic_.back(), cb));
            log_blocks_generic_.back()->start(uint8_t(
                100.0f / (float)freq)); // this is in tens of milliseconds
          }
        }
      }
  ...
  const Crazyflie::ParamTocEntry *paramTocEntry(const std::string &group,
  ...
  void change_parameter(const rclcpp::Parameter &p)
  {
    ...
    if (p.get_name().find(prefix) != 0)
    {
      RCLCPP_ERROR(logger_,
                   "[%s] Incorrect parameter update request for param \"%s\"",
                   name_.c_str(), p.get_name().c_str());
      return;
    }
    size_t pos = p.get_name().find(".", prefix.size());
    std::string group(p.get_name().begin() + prefix.size(),
                      p.get_name().begin() + pos);
    std::string name(p.get_name().begin() + pos + 1, p.get_name().end());

    RCLCPP_INFO(logger_, "[%s] Update parameter \"%s.%s\" to %s", name_.c_str(),
    ...
    {
      RCLCPP_ERROR(logger_, "[%s] Could not find param %s/%s", name_.c_str(),
                   group.c_str(), name.c_str());
    }
  }

private:
  void cmd_full_state_changed(
      const crazyflie_interfaces::msg::FullState::SharedPtr msg)
  {
    ...
    cf_.sendFullStateSetpoint(x, y, z, vx, vy, vz, ax, ay, az, qx, qy, qz, qw,
                              rollRate, pitchRate, yawRate);
  }

  void
  cmd_position_changed(const crazyflie_interfaces::msg::Position::SharedPtr msg)
  {
    float x = msg->x;
    float y = msg->y;
    float z = msg->z;
  ...
  void cmd_velocity_world_changed(
      const crazyflie_interfaces::msg::VelocityWorld::SharedPtr msg)
  {
    float vx = msg->vel.x;
    float vy = msg->vel.y;
    float vz = msg->vel.z;
  ...
  void cmd_hover_changed(const crazyflie_interfaces::msg::Hover::SharedPtr msg)
  {
    ...
    float vy = msg->vy;
    float yawRate =
        -1.0 * msg->yaw_rate * 180.0 / M_PI; // Convert from radians to degrees
  ...
  void cmd_vel_legacy_changed(const geometry_msgs::msg::Twist::SharedPtr msg)
  {
    ...
    uint16_t thrust =
        std::min<uint16_t>(std::max<float>(msg->linear.z, 0.0), 60000);
    // RCLCPP_INFO(logger_, "roll: %f, pitch: %f, yaw: %f, thrust: %u", roll,
    // pitch, yawrate, (unsigned int)thrust);
  ...
  {
    RCLCPP_INFO(logger_,
                "[%s] start_trajectory(id=%d, timescale=%f, reversed=%d, "
                "relative=%d, group_mask=%d)",
                name_.c_str(), request->trajectory_id, request->timescale,
                request->reversed, request->relative, request->group_mask);
    cf_.startTrajectory(request->trajectory_id, request->timescale,
                        request->reversed, request->relative,
                        request->group_mask);
  }
  ...
  {
    RCLCPP_INFO(
        logger_, "[%s] takeoff(height=%f m, duration=%f s, group_mask=%d)",
        name_.c_str(), request->height,
        rclcpp::Duration(request->duration).seconds(), request->group_mask);
    cf_.takeoff(request->height, rclcpp::Duration(request->duration).seconds(),
                request->group_mask);
  }
            ...
            std::shared_ptr<Land::Response> response)
  {
    RCLCPP_INFO(logger_, "[%s] land(height=%f m, duration=%f s, group_mask=%d)",
                ...
                request->group_mask);
    cf_.land(request->height, rclcpp::Duration(request->duration).seconds(),
             request->group_mask);
  }
  ...
  {
    RCLCPP_INFO(logger_,
                "[%s] go_to(position=%f,%f,%f m, yaw=%f rad, duration=%f s, "
                "relative=%d, group_mask=%d)",
                name_.c_str(), request->goal.x, request->goal.y,
                request->goal.z, request->yaw,
                rclcpp::Duration(request->duration).seconds(),
                request->relative, request->group_mask);
    cf_.goTo(request->goal.x, request->goal.y, request->goal.z, request->yaw,
             rclcpp::Duration(request->duration).seconds(), request->relative,
             request->group_mask);
  }

  void
  upload_trajectory(const std::shared_ptr<UploadTrajectory::Request> request,
                    std::shared_ptr<UploadTrajectory::Response> response)
  {
    ...
    for (size_t i = 0; i < pieces.size(); ++i)
    {
      ...
      pieces[i].duration =
          rclcpp::Duration(request->pieces[i].duration).seconds();
  ...
  void notify_setpoints_stop(
      const std::shared_ptr<NotifySetpointsStop::Request> request,
      std::shared_ptr<NotifySetpointsStop::Response> response)
  {
    RCLCPP_INFO(
        logger_,
        "[%s] notify_setpoints_stop(remain_valid_millisecs%d, group_mask=%d)",
  ...
  void on_logging_status(uint32_t time_in_ms, const logStatus *data)
  {
    if (publisher_status_)
    {
      ...
      size_t deltaTxUc =
          statsUc.sent_count - previous_stats_unicast_.sent_count;
      ...
      size_t deltaTxBc =
          statsBc.sent_count - previous_stats_broadcast_.sent_count;
      ...
      if (msg.num_rx_unicast > msg.num_tx_unicast * 1.05 /*allow some slack*/)
      {
        RCLCPP_WARN(
            logger_,
            "[%s] Unexpected number of unicast packets. Sent: %d. Received: %d",
            name_.c_str(), msg.num_tx_unicast, msg.num_rx_unicast);
      }
      if (msg.num_tx_unicast > 0)
      {
        float unicast_receive_rate =
            msg.num_rx_unicast / (float)msg.num_tx_unicast;
        if (unicast_receive_rate < min_unicast_receive_rate_)
        {
          RCLCPP_WARN(logger_,
                      "[%s] Low unicast receive rate (%.2f < %.2f). Sent: %d. "
                      "Received: %d",
                      name_.c_str(), unicast_receive_rate,
                      min_unicast_receive_rate_, msg.num_tx_unicast,
                      msg.num_rx_unicast);
        }
      }

      if (msg.num_rx_broadcast >
          msg.num_tx_broadcast * 1.05 /*allow some slack*/)
      {
        RCLCPP_WARN(logger_,
                    "[%s] Unexpected number of broadcast packets. Sent: %d. "
                    "Received: %d",
                    name_.c_str(), msg.num_tx_broadcast, msg.num_rx_broadcast);
      }
      if (msg.num_tx_broadcast > 0)
      {
        float broadcast_receive_rate =
            msg.num_rx_broadcast / (float)msg.num_tx_broadcast;
        if (broadcast_receive_rate < min_broadcast_receive_rate_)
        {
          RCLCPP_WARN(logger_,
                      "[%s] Low broadcast receive rate (%.2f < %.2f). Sent: "
                      "%d. Received: %d",
                      name_.c_str(), broadcast_receive_rate,
                      min_broadcast_receive_rate_, msg.num_tx_broadcast,
                         ...
                         void *userData)
  {
    ...
    auto pub = reinterpret_cast<rclcpp::Publisher<
        crazyflie_interfaces::msg::LogDataGeneric>::SharedPtr *>(userData);
  ...
  void on_link_statistics_timer()
  {
    ...
    {
      RCLCPP_WARN(logger_, "[%s] last latency update: %f s", name_.c_str(),
                  elapsed.count());
    }
    ...
    if (stats.ack_count > 0)
    {
      ...
      {
        RCLCPP_WARN(logger_, "[%s] Ack rate: %.1f %%", name_.c_str(),
                    ack_rate * 100);
      }
    }
  ...
  void on_latency(uint64_t latency_in_us)
  {
    ...
    {
      RCLCPP_WARN(logger_, "[%s] High latency: %.1f ms", name_.c_str(),
                  latency_in_us / 1000.0);
    }
...
private:
  ...
  rclcpp::Service<NotifySetpointsStop>::SharedPtr
      service_notify_setpoints_stop_;
  rclcpp::Service<Arm>::SharedPtr service_arm_;

  rclcpp::Subscription<geometry_msgs::msg::Twist>::SharedPtr
      subscription_cmd_vel_legacy_;
  rclcpp::Subscription<crazyflie_interfaces::msg::FullState>::SharedPtr
      subscription_cmd_full_state_;
  rclcpp::Subscription<crazyflie_interfaces::msg::Position>::SharedPtr
      subscription_cmd_position_;
  rclcpp::Subscription<crazyflie_interfaces::msg::VelocityWorld>::SharedPtr
      subscription_cmd_velocity_world_;
  rclcpp::Subscription<crazyflie_interfaces::msg::Hover>::SharedPtr
      subscription_cmd_hover_;
  ...
  rclcpp::Publisher<crazyflie_interfaces::msg::Status>::SharedPtr
      publisher_status_;
  ...
  std::list<
      rclcpp::Publisher<crazyflie_interfaces::msg::LogDataGeneric>::SharedPtr>
      publishers_generic_;
  ...
  bool publish_stats_;
  rclcpp::Publisher<crazyflie_interfaces::msg::ConnectionStatisticsArray>::
      SharedPtr publisher_connection_stats_;
};

class CrazyflieServer : public rclcpp::Node
{
public:
  CrazyflieServer() : Node("crazyflie_server"), logger_(get_logger())
  {
    ...
    this->declare_parameter("firmware_params.query_all_values_on_connect",
                            false);

    broadcasts_num_repeats_ = this->get_parameter("all.broadcasts.num_repeats")
                                  .get_parameter_value()
                                  .get<int>();
    broadcasts_delay_between_repeats_ms_ =
        this->get_parameter("all.broadcasts.delay_between_repeats_ms")
            .get_parameter_value()
            .get<int>();
    ...
    float freq = this->get_parameter("warnings.frequency")
                     .get_parameter_value()
                     .get<float>();
    if (freq >= 0.0)
    {
      watchdog_timer_ = this->create_wall_timer(
          ...
          std::bind(&CrazyflieServer::on_watchdog_timer, this),
          callback_group_all_srv_);
    }
    this->declare_parameter("warnings.motion_capture.warning_if_rate_outside",
                            std::vector<double>({80.0, 120.0}));
    auto rate_range =
        this->get_parameter("warnings.motion_capture.warning_if_rate_outside")
            .get_parameter_value()
    ...
    this->declare_parameter("warnings.communication.min_unicast_receive_rate",
                            0.9);
    this->declare_parameter("warnings.communication.min_broadcast_receive_rate",
                            0.9);
    this->declare_parameter("warnings.communication.publish_stats", false);

    publish_stats_ = this->get_parameter("warnings.communication.publish_stats")
                         .get_parameter_value()
                         .get<bool>();
    if (publish_stats_)
    {
      publisher_connection_stats_ = this->create_publisher<
          crazyflie_interfaces::msg::ConnectionStatisticsArray>(
    ...
    {
      bool enabled =
          parameter_overrides.at("robots." + name + ".enabled").get<bool>();
      if (enabled)
      {
        // Lookup type
        std::string cf_type = parameter_overrides.at("robots." + name + ".type")
                                  .get<std::string>();
        // Find the connection setting for the given type
        const auto con =
            parameter_overrides.find("robot_types." + cf_type + ".connection");
        ...
        const auto mocap_en = parameter_overrides.find(
            "robot_types." + cf_type + ".motion_capture.enabled");
        ...
        else if (constr == "none")
        {
          ...
          uint8_t id =
              parameter_overrides.at("robots." + name + ".id").get<uint8_t>();
          update_name_to_id_map(name, id);
        }
        else
        {
          RCLCPP_INFO(logger_, "[all] Unknown connection type %s",
                      constr.c_str());
        }
      }
    }
    ...
    double poses_qos_deadline = this->get_parameter("poses_qos_deadline")
                                    .get_parameter_value()
                                    .get<double>();

    rclcpp::SensorDataQoS sensor_data_qos;
    sensor_data_qos.keep_last(1);
    sensor_data_qos.deadline(
        rclcpp::Duration(0 /*s*/, 1e9 / poses_qos_deadline /*ns*/));
    sub_poses_ = this->create_subscription<NamedPoseArray>(
        "poses", sensor_data_qos,
    ...
    cb_handle_ = param_subscriber_->add_parameter_event_callback(
        std::bind(&CrazyflieServer::on_parameter_event, this, _1));

    // topics for "all"
    subscription_cmd_full_state_ =
        this->create_subscription<crazyflie_interfaces::msg::FullState>(
            "all/cmd_full_state", rclcpp::SystemDefaultsQoS(),
            std::bind(&CrazyflieServer::cmd_full_state_changed, this, _1),
            sub_opt_all_cmd);

    // services for "all"
    service_start_trajectory_ = this->create_service<StartTrajectory>(
        "all/start_trajectory",
        std::bind(&CrazyflieServer::start_trajectory, this, _1, _2),
        get_service_qos(), callback_group_all_srv_);
    service_takeoff_ = this->create_service<Takeoff>(
        "all/takeoff", std::bind(&CrazyflieServer::takeoff, this, _1, _2),
        get_service_qos(), callback_group_all_srv_);
    service_land_ = this->create_service<Land>(
        "all/land", std::bind(&CrazyflieServer::land, this, _1, _2),
        get_service_qos(), callback_group_all_srv_);
    service_go_to_ = this->create_service<GoTo>(
        "all/go_to", std::bind(&CrazyflieServer::go_to, this, _1, _2),
        get_service_qos(), callback_group_all_srv_);
    service_notify_setpoints_stop_ = this->create_service<NotifySetpointsStop>(
        "all/notify_setpoints_stop",
        std::bind(&CrazyflieServer::notify_setpoints_stop, this, _1, _2),
        get_service_qos(), callback_group_all_srv_);
    service_arm_ = this->create_service<Arm>(
        "all/arm", std::bind(&CrazyflieServer::arm, this, _1, _2),
        get_service_qos(), callback_group_all_srv_);

    // This is the last service to announce and can be used to check if the
    // server is fully available
    service_emergency_ = this->create_service<Empty>(
        "all/emergency", std::bind(&CrazyflieServer::emergency, this, _1, _2),
        get_service_qos(), callback_group_all_srv_);
  }
...
private:
                 ...
                 std::shared_ptr<Empty::Response> response)
  {
    ...
    for (int i = 0; i < broadcasts_num_repeats_; ++i)
    {
      ...
      std::this_thread::sleep_for(
          std::chrono::milliseconds(broadcasts_delay_between_repeats_ms_));
    }
  }
  ...
  {
    RCLCPP_INFO(logger_,
                "[all] start_trajectory(id=%d, timescale=%f, reversed=%d, "
                "group_mask=%d)",
                request->trajectory_id, request->timescale, request->reversed,
                request->group_mask);
    for (int i = 0; i < broadcasts_num_repeats_; ++i)
    {
      for (auto &bc : broadcaster_)
  ...
  {
    RCLCPP_INFO(logger_,
                "[all] takeoff(height=%f m, duration=%f s, group_mask=%d)",
                request->height, rclcpp::Duration(request->duration).seconds(),
                request->group_mask);
    for (int i = 0; i < broadcasts_num_repeats_; ++i)
    {
      for (auto &bc : broadcaster_)
  ...
  {
    RCLCPP_INFO(logger_,
                "[all] land(height=%f m, duration=%f s, group_mask=%d)",
                request->height, rclcpp::Duration(request->duration).seconds(),
                request->group_mask);
    for (int i = 0; i < broadcasts_num_repeats_; ++i)
    {
  ...
  {
    RCLCPP_INFO(logger_,
                "[all] go_to(position=%f,%f,%f m, yaw=%f rad, duration=%f s, "
                "group_mask=%d)",
    ...
    for (int i = 0; i < broadcasts_num_repeats_; ++i)
    {
      ...
      {
        auto &cfbc = bc.second;
        cfbc->goTo(request->goal.x, request->goal.y, request->goal.z,
                   request->yaw, rclcpp::Duration(request->duration).seconds(),
                   request->group_mask);
      }
      std::this_thread::sleep_for(
          std::chrono::milliseconds(broadcasts_delay_between_repeats_ms_));
      ...
      std::shared_ptr<NotifySetpointsStop::Response> response)
  {
    RCLCPP_INFO(
        logger_,
        "[all] notify_setpoints_stop(remain_valid_millisecs%d, group_mask=%d)",
      ...
      const crazyflie_interfaces::msg::FullState::SharedPtr msg)
  {
    ...
    {
      auto &cfbc = bc.second;
      cfbc->sendFullStateSetpoint(x, y, z, vx, vy, vz, ax, ay, az, qx, qy, qz,
                                  qw, rollRate, pitchRate, yawRate);
    }
  }

  void posesChanged(const NamedPoseArray::SharedPtr msg)
  {
    mocap_data_received_timepoints_.emplace_back(
    ...
    for (const auto &pose : msg->poses)
    {
      ...
      if (iter != name_to_id_.end())
      {
        ...
        {
          data_position.push_back({id, (float)pose.pose.position.x,
        ...
        {
          data_pose.push_back(
              {id, (float)pose.pose.position.x, (float)pose.pose.position.y,
  ...
  void on_parameter_event(const rcl_interfaces::msg::ParameterEvent &event)
  {
    if (event.node == "/crazyflie_server")
    {
      ...
      for (auto &p : params)
      {
        ...
        std::string cfname(p.get_name().begin(),
                           p.get_name().begin() + params_pos);
        size_t prefixsize = params_pos + 8;
        if (cfname == "all")
        {
          size_t pos = p.get_name().find(".", prefixsize);
          std::string group(p.get_name().begin() + prefixsize,
                            p.get_name().begin() + pos);
          std::string name(p.get_name().begin() + pos + 1, p.get_name().end());
  ...
  void on_watchdog_timer()
  {
    ...
    if (mocap_data_received_timepoints_.size() >= 2)
    {
      ...
      for (size_t i = 0; i < mocap_data_received_timepoints_.size() - 1; ++i)
      {
        std::chrono::duration<double> diff =
            mocap_data_received_timepoints_[i + 1] -
            mocap_data_received_timepoints_[i];
      ...
      {
        RCLCPP_WARN(logger_,
                    "[all] Motion capture rate off (#: %d, Avg: %.1f, Min: "
                    "%.1f, Max: %.1f)",
    ...
    if (publish_stats_)
    {
      ...
      msg.header.frame_id =
          "world"; // this is across broadcasters, which is not directly
                   // associated with single CFs; hence keep "world" here
  ...
  template <class T>
  void broadcast_set_param(const std::string &group, const std::string &name,
                           const T &value)
  {
    for (int i = 0; i < broadcasts_num_repeats_; ++i)
    {
  ...
  void update_name_to_id_map(const std::string &name, uint8_t id)
  {
    ...
    {
      RCLCPP_WARN(logger_,
                  "[all] At least two objects with the same id (%d, %s, %s)",
                  id, name.c_str(), iter->first.c_str());
    }
    else
    {
...
private:
  ...
  std::vector<std::chrono::time_point<std::chrono::steady_clock>>
      mocap_data_received_timepoints_;
  bool publish_stats_;
  rclcpp::Publisher<crazyflie_interfaces::msg::ConnectionStatisticsArray>::
      SharedPtr publisher_connection_stats_;
