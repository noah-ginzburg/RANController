...
class ViconBridge(Node):
    def __init__(self):
        ...
        # Subscribe unconditionally -- a subscription for a topic that doesn't
        # exist yet just waits for a publisher; there's no need to (and, checking
        # get_topic_names_and_types() only once here, no reliable way to) confirm
        # the vicon_client topic is already up before this node's own construction
        # finishes discovering the graph.
    ...
    def vicon_callback(self, msg: Position):
        ...
        #     f"R: {msg.x_rot_euler * 180/3.14159:.4f}, "
        #     f"P: {msg.y_rot_euler * 180/3.14159:.4f}, "
        #     f"Y: {msg.z_rot_euler * 180/3.14159:.4f}")
...
def main(args=None):
    # The FIRST Ctrl+C must not stop the pose stream. On SIGINT the controller
    # commands a land and keeps the drone in the air for LAND_DURATION (5s);
    # crazyflie_server needs /poses for the whole descent, because the onboard
    # EKF has no other absolute reference. With rclpy's default handlers this
    # node exited immediately on Ctrl+C, so the drone finished landing blind on
    # gyro/accel dead reckoning -- exactly the runaway signature.
    #
    # So: SIGINT is deliberately NOT a shutdown here. We exit on SIGTERM, which
    # ros2 launch sends sigterm_timeout after Ctrl+C (kept above LAND_DURATION in
    # real_drone.launch.py). A second Ctrl+C still kills us, matching the
    # controller's second-Ctrl+C emergency stop -- once the motors are cut,
    # mocap no longer matters -- and keeps the node killable when run by hand.
    rclpy.init(args=args, signal_handler_options=SignalHandlerOptions.NO)
    vicon_bridge = ViconBridge()

    def _first_sigint(_signum, _frame):
        vicon_bridge.get_logger().info(
            'Ctrl+C: staying up so the drone has mocap while it lands. '
            'Exits on SIGTERM from launch, or a second Ctrl+C.')
